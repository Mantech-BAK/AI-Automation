...
router.post('/:id/tasks/add', async (req, res) => {
  try {
    const { id } = req.params;
    const { task_name, task_type, expiry_date, registration_date, reminder_days } = req.body;
    ...
    const { rows } = await pool.query(
      `INSERT INTO vehicle_tasks (vehicle_id, task_name, task_type, expiry_date, registration_date, reminder_days)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [id, task_name, task_type, expiry_date || null, registration_date || null, reminder_days || 30]
    );
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE vehicle_tasks SET
       ...
       WHERE id = $6
       RETURNING *`,
      [task_name || null, task_type || null, expiry_date || null, registration_date || null, reminder_days || null, task_id]
    );
...
router.put('/tasks/:task_id/complete', async (req, res) => {
  try {
    ...
    const reminderDays = task.reminder_days || 30;

    // Close out this cycle as history (keeps its original expiry_date on
    // record) and start a fresh cycle in a new row - the same pattern the
    // rest of the system uses for document/equipment renewals, so the daily
    // check's dedup logic (which only looks at not-yet-completed rows) keeps
    // working without any special-casing.
    await pool.query(
      `UPDATE vehicle_tasks SET status = 'completed', completed_at = NOW() WHERE id = $1`,
      [task_id]
    );

    const { rows } = await pool.query(
      `INSERT INTO vehicle_tasks (vehicle_id, task_name, task_type, expiry_date, registration_date, reminder_days)
       VALUES ($1, $2, $3, CURRENT_DATE + ($4 * 12) * INTERVAL '1 day', CURRENT_DATE, $4)
       RETURNING *`,
      [task.vehicle_id, task.task_name, task.task_type, reminderDays]
    );
